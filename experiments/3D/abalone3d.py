...
# Define columns
feature_columns = ['Length', 'Whole weight']  # Selected features for fitting the hyperplane
target_column = 'Rings'  # Target variable for distance-based R² calculation

# Generate a regression dataset with 6 features
data = pd.read_csv(r"D:\course\P\Project4\pythonProject\3\abalone.csv")


data_selected = data[['Length', 'Whole weight', 'Rings']]
sampled_data = data_selected.sample(n=300, random_state=1)
data_train, data_test = train_test_split(sampled_data, test_size=0.3, random_state=0)
