...
# ===== 数据集 =====
feature_columns = ['Length1', "Length2", "Height"]  # 特征
target_column = 'Weight'  # 目标变量

data = pd.read_csv(r"D:\course\P\Project4\pythonProject\3\Fish.csv")
data_selected = data[feature_columns + [target_column]]
data_sampled = data_selected.sample(n=100, random_state=0)
data_train, data_test = train_test_split(data_sampled, test_size=0.3, random_state=0)

# **Step 1: 选择 Tukey + PCA 选出的点**
selected_points = select_tukey_pca_combined_points(data_train, feature_columns, target_column)

# **Step 2: 拟合 Tukey-based 超平面**
X_selected = selected_points[feature_columns].values
y_selected = selected_points[target_column].values
tukey_median_values = np.column_stack((X_selected, y_selected))
...
if normal_vector is not None:
    ...
    numerator = np.abs(np.dot(X_test, normal_vector[:-1]) + intercept)
    denominator = np.linalg.norm(normal_vector[:-1])
    distances = numerator / denominator  # 计算归一化距离

    # **计算误差度量**
    mae_tukey = np.mean(distances)  # Tukey-based MAE
    r2_tukey = 1 - (np.var(distances) / np.var(np.abs(y_test - np.mean(y_test))))  # Tukey-based R²

    print(f"Tukey-based Fit -> MAE: {mae_tukey:.4f}, R²: {r2_tukey:.4f}")
else:
    print("❌ Tukey-based fit failed. Setting default MAE/R².")
    mae_tukey, r2_tukey = None, None  # 确保变量被定义

# **Step 4: 传统 Linear Regression**
